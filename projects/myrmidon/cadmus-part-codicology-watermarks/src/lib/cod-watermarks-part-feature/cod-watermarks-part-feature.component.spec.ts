...
describe('CodWatermarksPartFeatureComponent', () => {
  const PART = buildPart<CodWatermarksPart>(COD_WATERMARKS_PART_TYPEID, {
    watermarks: [{ name: 'crown', description: 'a crown', chronotopes: [{ place: { value: 'Fabriano' } }] }],
  });
  ...
  async function setup(options?: { roleId?: string }) {
    ...
    const result = await render(CodWatermarksPartFeatureComponent, {
