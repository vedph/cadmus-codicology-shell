...
describe('CodEditsPartFeatureComponent', () => {
  const PART = buildPart<CodEditsPart>(COD_EDITS_PART_TYPEID, {
    edits: [{ type: 'gloss', ranges: [{ start: { n: 1 }, end: { n: 2 } }], text: 'hello' }],
  });
  ...
  async function setup(options?: { roleId?: string }) {
    ...
    const result = await render(CodEditsPartFeatureComponent, {
